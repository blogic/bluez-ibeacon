...
int enable_advertising(int advertising_interval, char *advertising_uuid,
		       int major_number, int minor_number, int rssi_value)
{
...
void main(int argc, char **argv)
{
	if (argc != 6) {
		fprintf(stderr,
			"Usage: %s <advertisement time in ms> <UUID> <major number> <minor number> <RSSI calibration amount>\n",
			argv[0]);
		exit(1);
	}

	int rc = enable_advertising(atoi(argv[1]), argv[2], atoi(argv[3]),
